...
| `0009-learning-quiz-clear-feedback-on-advance.patch` | learning quiz bugfix | Fixes the Learning quiz "Next does nothing" bug. The quiz feedback (explanation + Next button) is rendered as a **separate Preact root** into `#quiz-feedback` and shown imperatively (`display:block`). Re-rendering the quiz card on Skip/Next does not clear it — the card's own `#quiz-feedback` vnode is an empty `display:none` leaf, so the parent reconciliation leaves the prior question's feedback (and its stale Next button) in place; clicking that stale Next re-renders the *current* question, so it appears to do nothing (and the visible explanation belongs to the previous question). Adds an exported `clearQuizFeedback()` (unmounts the separate root + re-hides the panel) and calls it on both advance paths in `wireQuizHandlers`. Regression-tested by `sidecar/src/quiz-next-advance.test.ts`. Environment-independent upstream bug; fix is upstreamable. |
