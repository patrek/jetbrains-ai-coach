...
| `0006-webview-llm-capability-gate.patch` | part 6; grown by cli-provider | The one sanctioned webview divergence for capability gating (decision D6, ADR 0006). `shared.ts` learns host LLM availability via `getCapabilities`, gates the 12 LLM-dependent methods before they fire when `llm === false`, and translates the sidecar's `llm-unavailable` error into a friendly "ask Claude Code to do it directly" message — so every degraded LLM *generation* feature shows clear guidance instead of a hang or a raw error (and does not falsely imply an analytics MCP tool generates it). **cli-provider growth (deliberate, risk-flagged):** the two provider-backed methods (`generateRule`, `explainOccurrence`) now gate on `capabilities.provider.status` instead of the single `llm` flag — each provider state (not-selected / not-consented / not-installed / unauthenticated) and runtime failure `reason` (timeout / cli-error / bad-output) renders a distinct, actionable message — and they re-poll `getCapabilities` per call so a settings change takes effect without a host→webview push. The other 10 methods keep the single-flag degraded UX. Upstreamable: a VS Code host answers `{llm:true}` (+ `provider.status:'active'`) and the gate is inert. See `docs/llm-degradation-audit.md` for the per-page hide-vs-disable decisions. |
