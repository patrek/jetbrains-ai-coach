...
| `0009-learning-quiz-fresh-card-per-question.patch` | learning quiz bugfix | Fixes two same-family Learning quiz bugs from Preact reusing the quiz card's DOM nodes across questions, so imperative mutations from answering question N leak onto N+1: (1) the feedback panel (explanation + Next button), rendered as a separate Preact root into `#quiz-feedback` and shown imperatively, stayed visible — and its stale Next button re-rendered the *current* question ("Next does nothing", with a mismatched explanation); (2) the choice buttons stayed `disabled` with their correct/wrong classes, so the next question could not be answered (no answer → no Next button). The Code Review game ("Slop or Not", `renderCodeReviewRound`) has the identical defect: the snippets keep their `learn-cr-disabled` class so the next round is unpickable. Fix: add `key=${index}` to both the quiz card (`renderQuiz`) and the round card (`renderCodeReviewRound`) so each item mounts a **fresh card** (fresh enabled choices/snippets + an empty feedback panel), eliminating all carry-over at the root. Regression-tested by `sidecar/src/quiz-next-advance.test.ts` (full walkthroughs of both games). Environment-independent upstream bug; fix is upstreamable. |
